defmodule QueryTest do
  ...
  test "search for specific term" do
    result =
      Search.search(
        %Query.Term{fields: [:title], value: "Pojke"},
        [@test_index]
      )
  ...
  test "on_fields" do
    locs =
      Enum.into(
        [%Location{field: :body, position: 1}, %Location{field: :title, position: 1}],
        MapSet.new()
      )
  ...
  test "search for term with multiple hits" do
    result =
      Search.search(
        %Query.Term{fields: [:body], value: "att"},
        [:foo, @test_index]
      )
  ...
  test "term query obeys field parameter" do
    result =
      Search.search(
        %Query.Term{fields: [:body], value: "cedrik"},
        [@test_index]
      )
  ...
  test "term query on many fields" do
    r =
      Search.search(
        %Query.Term{fields: [:title, :body], value: "cedrik"},
        [@test_index]
      )
  ...
  test "term query without fields looks at all" do
    r =
      Search.search(
        %Query.Term{fields: [], value: "cedrik"},
        [@test_index]
      )
  ...
  test "boolean query ORs and ANDs" do
    ...
    r =
      Search.search(
        %Query.Boolean{optional: opt, must: must},
        [@test_index]
      )

    # TODO 1 should have higher ranking!
    assert r.hits |> T.ids() |> Enum.sort() == ["0", "1"]
  end
  ...
  test "boolean query NOTs + ANDs" do
    ...
    r =
      Search.search(
        %Query.Boolean{must: yep, must_not: nope},
        [@test_index]
      )
  ...
  test "nested stuff" do
    have = [
      %Query.Boolean{optional: [%Query.Term{value: "cedrik"}, %Query.Term{value: "Torslandafabriken"}]},
      %Query.Term{value: "a"}
    ]

    r =
      Search.search(
        %Query.Boolean{must: have},
        [@test_index]
      )
  ...
  test "ending wildcard query" do
    r =
      Search.search(
        %Query.Wildcard{fields: [:title], value: "Student*"},
        [@test_index]
      )
  ...
  test "leading wildcard query" do
    r =
      Search.search(
        %Query.Wildcard{fields: [:title], value: "*fabriken"},
        [@test_index]
      )
