defmodule Query.Boolean do
  ...
  defimpl Search, for: Query.Boolean do
    ...
    def search(query, indices) do
      nope = fn ->
        Search.search(
          %Query.Boolean{must: [], optional: query.must_not, must_not: []},
          indices
        )
      end
      ...
      res = fn ->
        Search.search(
          %Query.Boolean{must: query.must, optional: query.optional, must_not: []},
          indices
        )
      end
    ...
    def intersecting(hits1, hits2) do
      ...
      |> Stream.map(fn i ->
        {i,
         MapSet.union(
           Map.get(hs1, i, MapSet.new()),
           Map.get(hs2, i, MapSet.new())
         )}
      end)
      |> Enum.to_list()
    end
